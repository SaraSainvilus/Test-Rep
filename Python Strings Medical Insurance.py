...
total_ins = 0.0
for cost in ins_cost:
    total_ins += float(cost)
print(total_ins)    
...
average_ins = total_ins/len(ins_cost)
print(round(average_ins,1))
...
title_name = [name.title() for name in first_names]
for name in title_name:
    print(name)   

statements = []
index = 0
if index < (len(title_name)-1):
    for name in title_name:
        Statement = name + " is " + str(ages[index]) + " years old with a BMI of " + str(bmis[index]) + " and an insurance cost of $" + str(ins_cost[index]) + "."
